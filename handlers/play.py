...
async def playlist(client, message):
    ...
    msg = "**Dimainkan** in {}".format(message.chat.title)
    msg += "\n- "+ now_playing
    msg += "\n- Req Dari "+by
    temp.pop(0)
    if temp:
...
def updated_stats(chat, queue, vol=100):
    if chat.id in callsmusic.pytgcalls.active_calls:
        ...
        stats = 'Setting Untuk **{}**'.format(chat.title)
        if len(que) > 0:
            stats += '\n\n'
            stats += 'Volume : {}%\n'.format(vol)
            stats += 'Lagu Dalam Queue : `{}`\n'.format(len(que))
            stats += 'Dimainkan : **{}**\n'.format(queue[0][0])
            stats += 'Req Dari : {}'.format(queue[0][1].mention)
    else:
        stats = None
    return stats
...
async def p_cb(b, cb):
    ...
    if type_ == 'playlist':           
        ...
        if not queue:   
            await cb.message.edit('Tidak Ada Musik')
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
        by = temp[0][1].mention(style='md')
        msg = "**Dimainkan** in {}".format(cb.message.chat.title)
        msg += "\n- "+ now_playing
        msg += "\n- Req Dari "+by
        temp.pop(0)
        if temp:
...
async def m_cb(b, cb):
    ...
    if type_ == 'pause':
        if (
            ...
            await cb.answer('Chat Tidak Terkoneksi!', show_alert=True)
        else:
        ...
        if (
            chat_id not in callsmusic.pytgcalls.active_calls
                ...
                await cb.answer('Chat Tidak Terkoneksi Atau Lagu Telah Dimainkan', show_alert=True)
        else:
            callsmusic.pytgcalls.resume_stream(chat_id)
            await cb.answer('Musik Dilanjutkan!')     
    elif type_ == 'puse':         
        if (
            ...
            await cb.answer('Chat Tidak Terkoneksi Atau Lagu Telah Dijeda', show_alert=True)
        else:
            callsmusic.pytgcalls.pause_stream(chat_id)
        ...
        if chat_id not in callsmusic.pytgcalls.active_calls:
            ...
            if callsmusic.queues.is_empty(chat_id):
                ...
                await cb.message.edit('- Playlist Kosong..\n- Meninggalkan VCG!')
            else:
                ...
                await cb.message.reply_text(f'- Lagu Diskip\n- Dimainkan **{qeue[0][0]}**')
        ...
        if chat_id in callsmusic.pytgcalls.active_calls:
            ...
            callsmusic.pytgcalls.leave_group_call(chat_id)
            await cb.message.edit('Sukses Meninggalkan Chat!')
        else:
            await cb.answer('Chat Tidak Terkoneksi!', show_alert=True)
...
async def play(_, message: Message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    await lel.edit("⏳ **Memutar Lagu Permintaan Anda...**")
    ...
    try:
        ...
        await lel.edit("Lagu Tak Ditemukan. Coba Lagu Lain Atau Ketik Dengan Benar.")
        print(str(e))
        return
...
async def deezer(client: Client, message_: Message):
    ...
    try:
        ...
        await res.edit(
            "Tidak Menemukan Apapun, Ketik Dengan Benar!"
        )
        is_playing = False
        return
    keyboard = InlineKeyboardMarkup(
...
async def jiosaavn(client: Client, message_: Message):
    ...
    try:
        ...
        await lel.edit(
            "<i> Assistance Tidak Ada Digrup, Minta Admin Untuk /play Command Untuk Pertama Kali Atau Tambahkan Asisten Secara Manual</i>"
        )
        return     
    requested_by = message_.from_user.first_name
    chat_id=message_.chat.id
    ...
    query = text[1]
    res = lel
    await res.edit(f"Mencari `{query}` Via Jio Saavn")
    try:
        async with aiohttp.ClientSession() as session:
